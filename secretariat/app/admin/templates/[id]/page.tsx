...
export default function EditTemplatePage() {
  ...
  const fields = template
    ? Object.entries(template.coord_map.fields ?? {}).sort(([a], [b]) => a.localeCompare(b))
    : []
  ...
  return (
    <div className="min-h-screen bg-[#f3f6ff]">
      <header className="bg-white border-b px-6 py-4">
        ...
        <p className="text-xs text-[#94afd5]">v{template?.version} · {template?.status}</p>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        ...
        <section className="bg-white rounded-xl border p-6 space-y-4">
          <h2 className="font-semibold text-[#12304f]">Details</h2>
          <div>
          ...
          <button
            ...
            className="bg-[#006092] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#004d75] disabled:opacity-50"
          >
            {saving ? 'Saving…' : 'Save'}
          </button>
        </section>
        ...
        <section className="bg-white rounded-xl border p-6">
          <h2 className="font-semibold text-[#12304f] mb-4">Fields ({fields.length})</h2>
          {fields.length === 0 ? (
            <p className="text-sm text-[#94afd5]">No fields calibrated yet. Use the Calibrate tool to set field positions.</p>
          ) : (
            <div className="overflow-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-xs text-[#94afd5] border-b">
                ...
                <tbody>
                  {fields.map(([key, def]) => (
                    <tr key={key} className="border-b last:border-0">
                      <td className="py-2 pr-4 font-mono text-[#12304f]">{key}</td>
                      <td className="py-2 pr-4 text-[#94afd5]">{(def as FieldDef).type}</td>
                      <td className="py-2 pr-4 text-[#94afd5]">{((def as FieldDef).page ?? 0) + 1}</td>
                      <td className="py-2 pr-4 text-[#94afd5]">{(def as FieldDef).position.x}</td>
                      <td className="py-2 pr-4 text-[#94afd5]">{(def as FieldDef).position.y}</td>
                      <td className="py-2 text-[#94afd5]">{(def as FieldDef).hidden ? 'No' : 'Yes'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="flex-1 bg-[#006092] text-white rounded-lg py-3 text-sm font-medium hover:bg-[#004d75]"
          >
            Calibrate
          </button>
          <button
            onClick={runTestFill}
            disabled={testing}
            className="flex items-center gap-1.5 px-5 rounded-lg py-3 text-sm font-medium border border-[#dde8f5] text-[#006092] hover:bg-[#eaf1ff] disabled:opacity-50"
          >
          ...
          {template?.status !== 'archived' && (
            <button
              ...
              className="px-5 rounded-lg py-3 text-sm font-medium border border-[#dde8f5] text-[#425d7f] hover:bg-[#f3f6ff]"
            >
              Archive
            </button>
          )}
        ...
        {testUrl && (
          <section className="bg-white rounded-xl border p-6 space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="font-semibold text-[#12304f]">Test Fill Preview</h2>
              <a href={testUrl} target="_blank" className="inline-flex items-center gap-1.5 text-sm text-[#006092] hover:underline">
