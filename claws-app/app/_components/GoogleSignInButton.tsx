...
export default function GoogleSignInButton({ label = 'Sign in with Google', next }: Props) {
  ...
  async function handleClick() {
    ...
    // Always derive callback from the live browser origin — avoids stale
    // NEXT_PUBLIC_APP_URL values baked into the build.
    // Default `next` to the current URL so the user comes back to the same
    // page (preserving query params like ?p=238802&autorun=1) after OAuth.
    const nextParam =
      next ??
      new URLSearchParams(window.location.search).get('next') ??
      `${window.location.pathname}${window.location.search}`
    const callback  = `${window.location.origin}/auth/callback?next=${encodeURIComponent(nextParam)}`
