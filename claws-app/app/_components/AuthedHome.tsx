...
export default function AuthedHome() {
  ...
  async function runLookup(postal: string, opts: { cacheOnly?: boolean; fromHistory?: boolean } = {}) {
    ...
    try {
      ...
      // postal code preserved so we auto-run after they sign in again.
      if (res.status === 401 && !opts.cacheOnly) {
        window.location.href = `/?p=${postal}&autorun=1`
        return
      }
  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-12">
      ...
      {/* Report */}
      {report && <TerritoryReportView report={report} />}
...
// and fresh-lookup count vs cap. Pulls from /api/me. Pricing /
// SGD-burn intentionally omitted — users shouldn't see cost numbers;
// those live in /admin only.
  ...
  return (
    <div className={`rounded-xl border px-4 py-2.5 mb-6 shadow-sm ${toneClasses[primary.tone]}`}>
      <div className="flex items-center justify-between gap-3 flex-wrap text-sm">
        ...
        {hasAccess && dailyCountMax > 0 && (
          <span className="text-xs font-mono">
            {dailyCount}/{dailyCountMax} today
          </span>
        )}
      </div>
    </div>
